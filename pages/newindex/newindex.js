...
Page({
      ...
      success: function (res) {
        ...
        qqmapsdk.reverseGeocoder({
          ...
          success: function (res) {
            ...
            //   that.setData({ shops: res.data.data, pageIndex: 0, hasMore: true })
            //   console.log(that.data.shops)
            // })
            return fetch('model/' + that.data.cityId).then(res => {
              console.log(res.data)
              res.data.data.forEach(item => {
                item.titleimage = JSON.parse(item.titleimage)
              })
              const totalCount = parseInt(res.data.msg)
              const hasMore = pageIndex * pageSize < totalCount
              that.setData({ shops: res.data.data, pageIndex: 0, hasMore: true })
  ...
  loadMore() {
    ...
    return fetch('home/list/product?cityId=' + that.data.cityId, params).then(res => {
      ...
      const pageSign = 0
      this.setData({ shops, pageIndex, hasMore, pageSign })
    })
  }
})
